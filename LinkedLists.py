# File: LinkedLists.py
# Description: Create linked list class that contains many methods related to ordered and unordered lists.
...
class LinkedList():
    ...
    def isOrdered(self):
        ...
        if self.getLength() == 0 or self.getLength == 1:
            return True
        else:
    ...
    def mergeList(self, list2):
        ...
        merList = self.copyList()
        
        current = list2.head
        
        while current != None:
            item = current.getData()
            merList.addInOrder(item)
            current = current.getNext()
    ...
    def isEqual(self, list2):
        ...
        if self.getLength() != list2.getLength():
            return False
        ...
        while (current1 != None and current2 != None) and equal == False:
